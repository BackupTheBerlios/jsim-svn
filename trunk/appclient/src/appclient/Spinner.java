...
//        startupStatus = new Status();
    ...
    public void startSpinner(IStatus status){
        id = status.getId();
        mode = status.getMode();
...
//        System.out.println("Update " + aRecord);
        startPoint.x = Center.x;
        startPoint.y = Center.y;        
        lineSpec.setStartPoint(startPoint);
        
        theRadians = getRadians(aRecord.getPosIndex());
        lineSpec.setRadians(theRadians);
        //endoint is ignored: it is set by the size of the panel in the GUI
